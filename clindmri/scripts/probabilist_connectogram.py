...
def create_logger(log_dir):
    ...
    # file logger
    log_filename = "probabilist_connectogram_%s.log" % time.strftime(
        "%Y-%m-%d_%H:%M:%S")
    if log_dir:
...
def main(outdir,
    ...
    subject_id: str
        the subject identifier. The subject should exist in
        the Freesurfer <SUBJECTS_DIR>.
    nodif_brain: str
        path to a preprocessed brain-only volume with bvalue=0.
    bedpostx_dir: str
        path to the bedpostx output directory.
    tracto_mask_type: str
    ...
    cortical_atlas: str (optional)
        the cortical atlas to use, "Desikan" (default) or "Destrieux".
    add_subcortical: bool (optional)
        if True include subcortical regions in the connectogram
        (Thalamus, Caudate, Putamen, Pallidum, Hippocampus,
        Amygdala, Accumbens-area, and VentralDC).
    nsamples: int (optional, default 5000)
        number of samples per voxel to initiate in seed region.
    nsteps: int (optional, default 2000)
        maximum number of steps for a sample.
    fs_subjects_dir: str (optional)
        the Freesurfer <SUBJECTS_DIR>. By default the Freesurfer
    ...
    # Use boundary based registration from Freesurfer to infer an affine
    # transformation between diffusion space and anatomy (T1)
    logger.info(
        "Registration of diffusion to T1 using Freesurfer bbregister...")
    ...
    logger.info("Running probtrackx2, nsamples: %i, nsteps: %i" %
                (nsamples, nsteps))
...
def get_cmd_line_args():
    ...
    """
    usage = ("%(prog)s <subject id> <nodif_brain> <bedpostx_dir> <outdir> "
             "[options]")
    ...
    parser.add_argument(
        ...
        help='The type of tractography mask to create, allowed types: '
             ...
             'based one a 6-connexity structuring element.')
    parser.add_argument(
        "--nsamples", type=int, default=5000, metavar="<nsamples>",
        help="Number of samples per voxel to initiate in seed "
             "region (default 5000).")
    parser.add_argument(
        "--nsteps", type=int, default=2000, metavar="<nsteps>",
        help="Maximum number of steps for a sample (default 2000).")
    parser.add_argument(
        "--fs-subjects-dir", metavar="<Freesurfer subjects directory>",
